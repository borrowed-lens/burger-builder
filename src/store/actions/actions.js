//actions starting with INITIATE added for sagas.

export const ADD_INGREDIENT = 'ADD_INGREDIENT';
export const REMOVE_INGREDIENT = 'REMOVE_INGREDIENT';
export const FETCH_INGREDIENTS_SUCCESS = 'FETCH_INGREDIENTS_SUCCESS';
export const FETCH_INGREDIENTS_ERROR = 'FETCH_INGREDIENTS_ERROR';
export const INITIATE_FETCH_INGREDIENTS = 'INITIATE_FETCH_INGREDIENTS';

export const CHECKOUT_START = 'CHECKOUT_START';
export const PLACE_ORDER_START = 'PLACE_ORDER_START';
export const PLACE_ORDER_SUCCESS = 'PLACE_ORDER_SUCCESS';
export const PLACE_ORDER_ERROR = 'PLACE_ORDER_ERROR';
export const FETCH_ORDERS_START = 'FETCH_ORDERS_START';
export const FETCH_ORDERS_SUCCESS = 'FETCH_ORDERS_SUCCESS';
export const FETCH_ORDERS_ERROR = 'FETCH_ORDERS_ERROR';
export const INITIATE_PLACE_ORDER = 'INITIATE_PLACE_ORDER';
export const INITIATE_FETCH_ORDERS = 'INITIATE_FETCH_ORDERS';

export const SET_AUTH_REDIRECT = 'SET_AUTH_REDIRECT';
export const AUTH_START = 'AUTH_START';
export const AUTH_SUCCESS = 'AUTH_SUCCESS';
export const AUTH_ERROR = 'AUTH_ERROR';
export const AUTH_CHECK = 'AUTH_CHECK';
export const INITIATE_LOGOUT = 'INITIATE_LOGOUT';
export const INITIATE_AUTH_TIMEOUT = 'INITIATE_AUTH_TIMEOUT';
export const INITIATE_AUTH = 'INITIATE_AUTH';
export const INITIATE_AUTH_CHECK = 'INITIATE_AUTH_CHECK';
export const LOGOUT = 'LOGOUT';



